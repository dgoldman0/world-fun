...
var contractAddress = 'TRYcXPKBQNrNfTW7eZT51oBQJjasNJWvru';
const TID = 1002567;
	...
	var run = function() {
		...
		contractGlobal.secondsSinceLastDrip().call().then(function (res) {
			...
			contractGlobal.dripsSinceLastPrize().call().then(function (res2) {
				...
				if (prizeAvail) {
					var tCtx = $("#color-details"), imageElem = $("#image");
					var perm = getPermutation(3, 3);
					var combi = colors[perm[0]] + "|" + colors[perm[1]] + "|" + colors[perm[2]];
				    tCtx.canvas.width = tCtx.measureText(combi).width;
				    tCtx.fillText(combi, 0, 10);
