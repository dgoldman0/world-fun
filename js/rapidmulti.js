...
$(document).ready(function() {
	...
	var run = function() {
		...
		contractGlobal.highestBurn1().call().then(function (res) {
			if (Number(res) != NaN)
				$('#first-burn').text(res / 1000000);
		});
		contractGlobal.highestBurn2().call().then(function (res) {
			if (Number(res) != NaN)
				$('#second-burn').text(res / 1000000);
		});
		contractGlobal.highestBurn3().call().then(function (res) {
			if (Number(res) != NaN)
				$('#third-burn').text(res / 1000000);
		});
		...
		if (checkConnection()) {
			...
			contract.checkRegistered().call().then(function (res) {
				if (res) {
					...
					$("#referral-link").html("Refer people and earn 5% of the TRX buy-in for every person that registers using <a href = \"http://worldbuilder.fun/rapid.php?ref=" + window.tronWeb.defaultAddress.base58 + "\">your referral link</a>.");
				} else {
					$("#register").show();
